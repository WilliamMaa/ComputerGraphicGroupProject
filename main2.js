...
import { color, depth, depthTexture, normalWorld, texture, float, normalMap, viewportSharedTexture, mx_worley_noise_float, positionWorld, timerLocal, MeshBasicNodeMaterial } from 'three/nodes';
...
sunLight.position.set(10, 100, -20);
...
waterMaterial.backdropNode = depthEffect.mul(3).min(2).mix(viewportTexture, viewportTexture.mul(color(0x9FC5E8)));
waterMaterial.backdropAlphaNode = depthEffect.oneMinus();
waterMaterial.transparent = true;
waterMaterial.normalNode = normalMap(texture(waterNormal));
waterMaterial.roughnessNode = float(0.5);
